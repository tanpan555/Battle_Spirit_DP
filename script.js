    ...
    if (p_type == 'CORE') {
      if (zone.includes("LIFE")) {
        ...
        life_list.innerHTML +=
          ...
          '<button class="btn_core" onclick="paid_core(\'LIFE\',-1,\''+player_side+'\')">-</button>' +
          '<button class="btn_core" onclick="paid_core(\'LIFE\',1,\''+player_side+'\')">+</button>' +
          '</div>';
      ...
      if (zone.includes("RESERVE")) {
        ...
        reserve_list.innerHTML +=
          ...
          '</div>' +
          '<div class="button-container" style="justify-content: center; margin-top: 20px;">' +
          '<button class="btn_core" onclick="paid_core(\'RESERVE\',-1,\''+player_side+'\')">-</button>' +
          '<button class="btn_core" onclick="paid_core(\'RESERVE\',1,\''+player_side+'\')">+</button>' +
          '</div>';
      ...
      if (zone.includes("FIELD") && player_board.CARD_IN_BOARD.length > 0) {
        ...
        for (let i = 0; i < player_board.CARD_IN_BOARD.length; i++) {
          field_list.innerHTML +=
            ...
            "</div>" +
            '<div class="button-container" style="justify-content: center; margin-top: 20px;">' +
            '<button class="btn_core" onclick="paid_core(\'FIELD\',-1,\''+player_side+'\')">-</button>' +
            '<button class="btn_core" onclick="paid_core(\'FIELD\',1,\''+player_side+'\')">+</button>' +
            "</div></div>";
        }
...
function calculateRemainingCost(cid,player_side) {
  ...
  let costReduct = getCostReduct(cid,player_side); // symbols required
  let boardSymbols = getAllBoardSymbols(player_side);
...
function paid_core(p_type,p_amt,player_side){
  let player_core_paid = (player_side == 1 ? player1_core_paid : player2_core_paid);
  let player_core = (player_side == 1? player1_core : player2_core);
  if(p_type == 'LIFE'){
    if(player_core_paid.core_life >= 0 && player_core_paid.core_life <= player_core.core_life){
      player_core_paid.core_life += p_amt;
      document.getElementById('P1_CORE_PAID_LIFE').innerHTML = ' ×' + player_core_paid.core_life;
    }
  }else if(p_type == 'RESERVE'){
    if(player1_core_paid.core_reserve >= 0 && player_core_paid.core_reserve <= player_core.core_reserve){
      player_core_paid.core_reserve += p_amt;
      document.getElementById('P1_CORE_PAID_RESERVE').innerHTML = ' ×' + player_core_paid.core_reserve;
    }
  }
